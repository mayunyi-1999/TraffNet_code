...
from datasetOp.PretrainDataset_sumosy import PreTrainRouteLearningDataset, collect_fn_preSelPath
from model.PreTrainRouteLearningModel_sumosy import PretrainRouteLearningModel
...
seqmaxsize = getSeqMaxSize(pathNodeDictFileName="../data/pathNodeDict_sumosy.txt")
...
for epoch in range(epochs):
    ...
    if not os.path.exists('pathSelParams_sumosy'):
        os.makedirs('pathSelParams_sumosy')
    with open('pathSelParams_sumosy/loss_sumosy.txt', 'a') as f:
        f.write(
    ...
    if min_val_total_loss > val_total_loss:
        torch.save(model, f'pathSelParams_sumosy/pathSel_sumosy.pkl')
