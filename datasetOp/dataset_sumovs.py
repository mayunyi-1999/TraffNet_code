...
def getDataGenerateOneStep(idx, labels, horizon):
    label = labels[idx + horizon - 1]
    folder = pathlib.Path(__file__).parent.parent.resolve()
    graphPath = os.path.join(folder, 'batchHtgGraph_sumovs')    # batch Heterograph path
...
class DataSetSumoVS(Dataset):
    def __init__(self,
                 ...
                 horizon,
                 start_idx):
        self.labels = np.load('../data_sumovs/labelsNp_sumovs.npz')['arr_0']
