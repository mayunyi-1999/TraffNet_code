...
def getLabel(t, window_width):
    areaflow = pd.read_csv("../data_sumovs/realflowAll14dayswhatIf.csv")
...
def getBatchGraph(t, window_width):
    ...
    for i in range(t, t + window_width):
        dat_i_G = dgl.load_graphs(f'../HTGWithFeat_sumovs/Hetero_{i}.bin', [0])[0][0]
        indat.append(dat_i_G)
    graphSeqATimeWindow = dgl.batch(indat)  # 6个图构成的图序列
    dgl.save_graphs(f"../batchHtgGraph_sumovs/batchHtg{t}.bin", [graphSeqATimeWindow])
...
if __name__ == '__main__':
    ...
    out_len = 1
    edgeNum = 254
    start_timestamps, end_timestamps = 0, 30 * 24 * 14
    ...
    label2Mins = getGraphLabels(start_timestamps=start_timestamps,
                                ...
                                out_len=out_len,
                                edgeNum=edgeNum)
    np.savez(f'../data_sumovs/labels_sumo_vs.npz', label2Mins)
    print('ok')
